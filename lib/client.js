	...
	factory: (require) => {
		...
		*/
		var CommandCodePricesController = class {
			...
			*/
				...
				call.call(this.remote).then((result) => {
					if (this.disposed) return;
					this.inFlight = false;
					if (result.ok) {
						this.publish({
		...
		*/
		function buildSessionCostView(input) {
			...
			if (!free && unpricedCacheWriteTokens > 0 && !pricedCosts.some((cost) => cost !== void 0 && cost > 0)) return;
			...
			const usageRemote = {
				...
				prices: async () => {
					...
					if (typeof call !== "function") return {
						ok: false,
						error: { message: "the Host serves no commandcode/prices endpoint" }
					};
