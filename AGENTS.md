...
src/command-guard.ts  The AI command guard: an answerer on the harness's
                      ...
                      only on a confident "safe" verdict, plus the hard
                      denylist, the thresholds, the execution/decision caches
                      and the transparent `tools/pre-execute` observer that
                      supplies the command the approval payload omits.
...
src/config-volatile.ts  The cross-generation volatile-config bridge:
                      `markVolatile()`/`markVolatileFields()` (feature-detect
                      schemastery's `.volatile()`; inert no-op on older
                      engines) and `unwrapVolatileConfig()` (reads frozen
                      `{ get() }` references per call, identity-preserving on
                      plain config). The reason `src/index.ts` builds the
                      Config fields TWICE — the marked `Config` for the loader
                      and 0.1.7's forms, the unmarked `LegacySettingsSchema`
                      for the ≤0.1.6 registration, whose base must also be
                      unwrapped.
src/client/settings-scope.ts  Self-contained `SettingsScope` over the
                      `remote.settings` wire (describe mirror + revision-fenced
                      mutate queue): the client half every generation speaks,
                      replacing the harness's ≤0.1.6 `settingsScope` service.
...
tests/volatile-config.test.ts the volatile bridge: marking degrades on old
                      schemastery, ref detection, per-call unwrap freshness,
...
tests/client-boot.test.ts client-boot integration tests (real apply() against a
                      ...
                      and the `remote.settings` scope path end to end: directory
                      read, invalidation re-read, a save over the path-op wire,
                      and the degraded profile without the transport).
...
- **Client bundle**: the package's `dsh.client` declaration (`platform: web`,
  ...
  `ctx.settingsScope` wrapper existed only through 0.1.6 and was removed by
  the 0.1.7 settings rewrite, while the describe/mutate wire underneath is
  identical from 0.1.2-rc.1 to 0.1.7-alpha.1 (same `SettingsDescribeValue`
  directory view, same mutate-answers-a-`SettingsNamespaceView` ROW contract,
  same `settings/document-updated` invalidation), so one implementation serves
  every supported engine; `settingsScope` must never reappear in the exported
  client `inject` list, because a service 0.1.7 does not provide would gate
  EVERY surface (page, card, usage card, panel, session cost) into silence.
  ...
  `tests/legacy-credentials.test.ts` pin these
...
- **dsh 0.1.7's settings rewrite, and the two-generation adaptation** (`src/config-volatile.ts`, the `ctx.inject(['settings'], …)` block in `src/index.ts`): 0.1.7 REPLACED the settings.yaml document with schema-derived profile Config forms (`SettingsProvider` → `SettingsForms`; `installSection` and the `settingsScope` client service are both gone), and its `settings.describe()` projects each entry through `volatileForm()` — a form contains ONLY schema nodes carrying `meta.volatile`, and a form edit whose path does not lie beneath a marked node is REFUSED. Six consequences are load-bearing:
  (1) **Every Config field except the top-level `apiKey` secret is marked via `markVolatileFields()`** — an unmarked field would be invisible to AND unwritable from the settings page on 0.1.7. The marking is feature-detected through a STRUCTURAL type (3.18.3 has `.volatile()`; this checkout pins 3.18.2, so the mark must degrade to a no-op instead of throwing during module load — never import the method from schemastery's types). `apiKey` stays unmarked on purpose: no settings surface writes it (page and TUI both write keys through the credentials seam), so a config-file edit to it reloading the fiber is correct for a secret literal. **The fields are built TWICE — `configFields()` is a factory, and `LegacySettingsSchema` must be a separate, UNMARKED schema.** That split is not tidiness: schemastery creates the frozen `{ get() }` references during PARSE on EVERY generation, and `dsh-settings` through 0.1.6 declares `schemastery: ^3.18.2`, so a freshly installed OLD engine resolves 3.18.3 and hands `apply()` references too — while that generation's `installSection` re-validates the base it is given (`register()` → `resolve()` → `schema(mergeLayers(base, section))`) and `structuredClone`s it for the directory. Handing it the marked schema plus the raw config is a BOOT-TIME `ValidationError` (`expected boolean but got [object Object]`) and a `DataCloneError` on the first directory read. So the legacy branch passes `LegacySettingsSchema` + `unwrapVolatileConfig(config)`; `npm run test:engine` audits both halves (check 6) and `--engine` against an old engine tree with schemastery ≥3.18.3 is the regression guard.
  (2) **`apply()` receives frozen `{ get() }` references for marked fields on that generation**, so `current()` is `unwrapVolatileConfig(source())` — a FRESH unwrap per read (a reference's identity is stable while its value changes, so a cached snapshot would go stale) that returns the input object UNTOUCHED when no field is a reference (preserving identity for the `options()` memo on ≤0.1.6). Only top-level fields are marked, so one unwrap level is complete. Everything downstream keeps reading a plain `Config`.
  (3) **Volatile writes commit in place — no remount — and notify the owning fiber** with `loader/volatile-update` (paths already committed; the event comes from `cordis-plugin-loader`, which is not a peer, so the listener is typed structurally and registered unconditionally — old loaders simply never emit it). The listener re-runs the two facts that are NOT re-derived from `current()` per use: the web-search selection (a written private field) and the dsh-TUI section re-registration. On ≤0.1.6 the SAME body runs from `installSection`'s `onChange`; a change to a NON-volatile field (only `apiKey`) reloads the fiber, which re-runs `apply` anyway. Never remove one branch "because the other covers it" — each covers its own generation.
  (4) **The Host registration is capability-branched, not version-branched**: `typeof service.installSection === 'function'` → the legacy path (which must STAY — ≤0.1.6's describe directory lists only installSection REGISTRATIONS, so dropping it would blank the settings page on every old engine); else `typeof service.configure === 'function'` → `configure({ auto: false }, ctx.fiber)` as an effect on the settings child (declares we ship our own page, so SettingsForms publishes no auto-generated form for this entry). Both faces are accessed through the local `SettingsServiceSeam`, because the installed dsh-settings .d.ts describes only one generation.
  (5) **The one-time 0.1.7 settings.yaml import needs no cooperation from us** — the engine looks the section up BY PROFILE ENTRY ID, and our section id and entry id are the SAME string (`llm-commandcode`). Never rename one without the other: a mismatch strands existing users' non-secret settings in `settings.yaml.imported` (logged, not applied). Pinned by `tests/volatile-config.test.ts` (the bridge), `tests/settings-scope.test.ts` (the client scope), `tests/package.test.ts` (four-way range sync + `0.1.7-alpha.1` in `dshReleases`) and `npm run test:engine`.
...
- **Isolated package install**: pnpm 10 auto-installs the package's DSH peers
  ...
  (read out of the 0.1.2-rc.1, 0.1.5-rc.2, 0.1.6-alpha.2 and 0.1.7-alpha.1 engines alike) — so
...
- **Models-page provider card (`settings.models.provider-card`)**: a keyed
  ...
  carries only a config hint (the settings.yaml hint through 0.1.6) over a
  permanently disabled apply — and shows
...
- **Wire protocol** (reverse-engineered, command-code@1.28.4; re-verified unchanged against 1.64.0):
  ...
  - Catalog: `GET {apiBase}/provider/v1/models` → `{ object: 'list', data: [{ id, name, context_length, supported_endpoints }] }`. The `supported_endpoints` member is new as of command-code@1.55.0/1.56.0 and is the authoritative per-model route list (63 × `['/chat/completions','/responses']` as of command-code@1.64.0 — 60 at 1.62.0 plus `gpt-6-sol` and `gpt-6-luna` — 9 Claude ids × `['/messages']` (`claude-opus-5-5` is the ninth), 8 × `['/chat/completions']`); this adapter ignores it and keeps the `claude-*` prefix rule in `requiresMessagesEndpoint()`, which agrees with it today.
  - Provider API endpoints (docs, 2026-09-23): `/provider/v1/chat/completions`, `/provider/v1/responses` (OpenAI + open models; added in command-code@1.55.0), `/provider/v1/messages` (Claude family only), `/provider/v1/models`, and `/provider/v1/systemone` (System One decisions, model `typesafe/jev` — new in the docs and in the CLI bundle at command-code@1.64.0; it is NOT a chat route, the model is absent from `/provider/v1/models`, and only `src/systemone.ts`'s own request path reaches it). This adapter's two chat transports stay the first plus `/alpha/generate`; neither is deprecated.
  ...
  - Defaults: `apiBase = https://api.commandcode.ai`, `COMMAND_CODE_CLI_VERSION = '1.64.0'`.
...
- **dsh-TUI settings section (`src/tui-settings.ts` + the optional
  ...
  settings-change hook (`installSection`'s `onChange` on ≤0.1.6, the
  `loader/volatile-update` listener from 0.1.7 — see the volatile-config
  bullet) — withdraw first, then declare, and only
