...
- **`npm run test:engine` is the only check that can see a bundle which cannot
  ...
  bundle's `require()` calls against the platform seed table, and the
  request-image policy generation the engine exposes. Run it before publishing
  and whenever a peer range or an engine version changes.
