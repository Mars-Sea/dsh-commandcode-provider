...
const finite$1 = (v) => typeof v === "number" && Number.isFinite(v) && v >= 0;
function tokensOf(value) {
	const u = record$2(value);
	if (!u || !finite$1(u.inputTokens) || !finite$1(u.outputTokens)) return void 0;
	if (u.cacheReadTokens !== void 0 && !finite$1(u.cacheReadTokens) || u.cacheWriteTokens !== void 0 && !finite$1(u.cacheWriteTokens)) return void 0;
	return {
...
function parseState(value) {
	...
	if (!s || !facts || !finite$1(facts.pricingKey) || !Array.isArray(facts.groups)) throw new TypeError("invalid commandCodeCost state");
	const group = (v) => {
		const g = record$2(v), t = record$2(g?.tokens);
		return !!g && typeof g.provider === "string" && typeof g.model === "string" && (g.at === null || finite$1(g.at)) && finite$1(g.contextTokens) && !!t && COST_TOKEN_KEYS.every((k) => finite$1(t[k]));
	};
	const sel = record$2(s.selection), last = record$2(s.last);
	if (!(s.selection === null || sel && typeof sel.provider === "string" && typeof sel.model === "string") || !(s.at === null || finite$1(s.at)) || !facts.groups.every(group) || !(s.last === null || last && finite$1(last.turn) && finite$1(last.step) && group(last.group))) throw new TypeError("invalid commandCodeCost state");
	return value;
}
	...
	return {
		...
		apply(state, event) {
			...
			if (event.type === "step/start" || event.type === "llm/retry-started") return {
				...state,
				at: finite$1(event.time) ? event.time : null,
				last: null
			};
			const tokens = usageOf(event.type, data);
			if (tokens === void 0 || !finite$1(data.turn) || !finite$1(data.step)) return state;
...
*/
const COMMAND_CODE_CLI_VERSION = "1.65.2";
...
var CommandCodeAdapter = class extends LlmAdapter {
	...
	*/
	async fetchEndpointJson(url, headers, timeoutMs = MODELS_TIMEOUT_MS) {
	...
	*/
	async getUsage(apiKey) {
		...
		const getJson = async (path) => {
			try {
				const { status, record, invalidResponse } = await this.fetchEndpointJson(`${base}${path}`, headers, connection.requestTimeoutMs);
				if (record === void 0) {
	...
	*/
	async probeWindowLimits(apiKey) {
		try {
			...
			for (const raw of [windowLimits.fiveHour, windowLimits.weekly]) {
				...
				exceeded = true;
				if (window.resetAt > 0 && window.resetAt - Date.now() <= MAX_TRUSTED_RESET_MS) resetAt = Math.max(resetAt, window.resetAt);
			}
	...
	async *stream(options) {
		...
		trace.record("response", {
			protocol,
			endpoint: protocol === "cli" ? `${connection.apiBase}/alpha/generate` : `${connection.apiBase}/provider/v1/chat/completions`,
			status: response.status,
			attempts: tried.size,
			maxTokens,
		...
		function* handle(event) {
			for (const chunk of handleEvent(asm, protocol, event)) if (chunk.type === "finish") finish = chunk;
		...
		try {
			for (;;) {
				...
				if (done) {
					...
					if (buffer.trim()) yield* handle(parseStreamEventLine(buffer));
					break;
				}
				...
				buffer = lines.pop() ?? "";
				for (const line of lines) yield* handle(parseStreamEventLine(line));
			...
			if (finish !== void 0) {
				...
				throw new LlmError(`Command Code API stream from ${connection.apiBase} ended before its finish event (${detail}) — the response was closed mid-generation, so this is not the model stopping by itself；Command Code API 流式响应在结束事件之前被关闭（生成中途断流），并非模型主动结束——请重试；若频繁出现，可用 ${STREAM_TRACE_ENV}=<文件路径> 抓取原始流以定位断流原因`, "STREAM_CLOSED");
			}
		} catch (error) {
...
function generateHttpError(status, errText, retryAfterMs) {
	...
	if (status === 422 && isZdrNoProviders(providerCode, providerDetail, errText)) return new LlmError(`Command Code API error 422 (${detail}): zero data retention is enforced but no ZDR-capable upstream is available for this model — turn the ZDR setting off (or use another model)；Command Code API 返回 422：已开启零数据保留（ZDR），但该模型当前没有可用的 ZDR 上游——请关闭 ZDR 开关，或改用其他模型`, "PROVIDER_HTTP_ERROR", { status: 422 });
...
function emptyCompletionError(reason, maxTokens, usage) {
	...
	if (mapFinishReason(reason).kind === "max-tokens") return new LlmError(`Command Code reached the output token limit without producing answer text or a tool call (${detail})；Command Code 已达到输出 token 上限，但没有生成正文或工具调用；请调整思考强度，或在模型和接口上限内增加输出预算`, "OUTPUT_TOKEN_LIMIT");
	if (reason === "content_filter" || reason === "content-filter") return new LlmError(`Command Code filtered the response (${detail})；Command Code 拦截了响应，未生成正文或工具调用`, "CONTENT_FILTER");
	return new LlmError(`Command Code finished without producing answer text or a tool call (${detail})；Command Code 返回了空响应（可能只有思考内容），重试通常可恢复`, "EMPTY_RESPONSE");
}
/**
...
*/
const MAX_TRUSTED_RESET_MS = 2592e6;
/**
...
function readWindowLimitEvidence(errText) {
	...
	const resetAtMs = published !== void 0 && Number.isFinite(published) && published - Date.now() <= MAX_TRUSTED_RESET_MS ? published : void 0;
	return {
...
var CommandCodeSearchProvider = class {
	...
	async search(request, signal) {
		...
		try {
			...
			if (signal?.aborted === true || isAbortError$1(error)) throw searchAborted(signal, error);
	...
	return {
		...
		fields: [
			...
			},
			{
				...
				hint: "typesafe/jev judges whether a shell command is safe; sandbox escalations also need separate scope and necessity verdicts. All must be confident enough; everything else asks as usual. Off by default.",
			...
			},
			{
				...
				options: [
					{
						value: "high",
						label: "High (0.95, approves least)",
						descriptions: { zh: "高（0.95，放行最少）" }
					},
					{
						value: "medium",
						label: "Medium (0.9)",
						descriptions: { zh: "中（0.9）" }
					},
					{
						value: "low",
						label: "Low (0.8, approves most)",
						descriptions: { zh: "低（0.8，放行最多）" }
					}
				],
...
const SYSTEMONE_ROUTE = "/provider/v1/systemone";
/** The one error type this client throws. Callers fail closed on it. */
var SystemOneError = class extends Error {
	kind;
	status;
	constructor(message, kind, status, options) {
...
*/
function deadlineOf(signal, timeoutMs) {
...
function withinDeadline(operation, deadline, timeoutMs) {
	return new Promise((resolve, reject) => {
		const onAbort = () => {
			reject(deadline.callerAborted() ? new SystemOneError("decision request aborted", "aborted") : new SystemOneError(`decision request timed out after ${timeoutMs}ms`, "timeout"));
		};
...
*/
function parseSystemOneAnswer(value) {
	if (typeof value !== "object" || value === null) return void 0;
	const raw = value;
	if (raw.type === "noul") {
		const noul = probability(raw.noul);
		return noul === void 0 ? void 0 : {
			type: "noul",
			noul
		};
	}
	if (raw.type === "choice") {
		if (typeof raw.choice !== "string" || raw.choice === "") return void 0;
		const confidence = raw.confidence === void 0 ? void 0 : probability(raw.confidence);
		if (raw.confidence !== void 0 && confidence === void 0) return void 0;
		const probabilities = raw.probabilities === void 0 ? void 0 : numberRecord(raw.probabilities);
		if (raw.probabilities !== void 0 && probabilities === void 0) return void 0;
		return {
			type: "choice",
			choice: raw.choice,
...
function parseSystemOneResponse(value) {
	...
	if (typeof rawAnswers !== "object" || rawAnswers === null || Array.isArray(rawAnswers)) return void 0;
	...
	const usage = raw.usage === void 0 ? void 0 : parseUsage(raw.usage);
	if (raw.usage !== void 0 && usage === void 0) return void 0;
	return {
...
function parseUsage(value) {
	if (typeof value !== "object" || value === null || Array.isArray(value)) return void 0;
	const raw = value;
	const inputTokens = raw.input_tokens === void 0 ? void 0 : finite(raw.input_tokens);
	if (raw.input_tokens !== void 0 && inputTokens === void 0) return void 0;
	const outputTokens = raw.output_tokens === void 0 ? void 0 : finite(raw.output_tokens);
	if (raw.output_tokens !== void 0 && outputTokens === void 0) return void 0;
	return {
...
function assertRequest(request) {
	...
	if (ids.length > 20) throw new SystemOneError(`a decision request carries at most 20 questions (got ${ids.length})`, "invalid-request");
	for (const id of ids) if (id.trim() === "") throw new SystemOneError("every decision question needs a non-empty id", "invalid-request");
}
/** Read the endpoint's error body into a one-line diagnosis. */
async function httpFailure(response) {
	let detail = "";
	try {
		const slice = (await response.text()).trim().slice(0, 200);
		if (slice !== "") detail = `: ${slice}`;
	} catch {}
	return new SystemOneError(`Command Code decision request failed (HTTP ${response.status})${detail}`, "http", response.status);
}
/**
...
*/
async function runSystemOne(deps, request, options = {}) {
	assertRequest(request);
	const apiBase = deps.apiBase();
	if (!URL.canParse(apiBase)) throw new SystemOneError(`Command Code decision request is misconfigured: apiBase ${JSON.stringify(apiBase)} is not a valid URL`, "invalid-request");
	...
	try {
		...
		try {
			...
			throw new SystemOneError(`Command Code decision request has no usable credential: ${error instanceof Error ? error.message : String(error)}`, "credential", void 0, { cause: error });
		}
		const body = {
			model: request.model ?? "typesafe/jev",
			state: request.state,
			questions: request.questions
		};
		if (deadline.signal.aborted) throw deadline.callerAborted() ? new SystemOneError("decision request aborted", "aborted") : new SystemOneError(`decision request timed out after ${timeoutMs}ms`, "timeout");
		let response;
		try {
			...
			if (deadline.callerAborted()) throw new SystemOneError("decision request aborted", "aborted", void 0, { cause: error });
			if (deadline.timedOut() || isAbortError(error)) throw new SystemOneError(`decision request timed out after ${timeoutMs}ms`, "timeout", void 0, { cause: error });
			throw new SystemOneError(`Command Code decision request failed: ${error instanceof Error ? error.message : String(error)}`, "network", void 0, { cause: error });
		}
		...
		try {
			...
			if (deadline.callerAborted()) throw new SystemOneError("decision request aborted", "aborted", void 0, { cause: error });
			throw new SystemOneError("Command Code returned an unparseable decision body", "invalid-response", void 0, { cause: error });
		}
		const parsed = parseSystemOneResponse(payload);
		if (parsed === void 0) throw new SystemOneError("Command Code returned a decision body this plugin cannot read", "invalid-response");
		return parsed;
	} finally {
		deadline.dispose();
	}
...
const COMMAND_GUARD_SHELL_TOOLS = ["bash", "pwsh"];
/** Question id the primary safety verdict is keyed by. */
const COMMAND_GUARD_QUESTION_ID = "safe";
/** Question id for the narrow-purpose part of a sandbox escalation. */
const COMMAND_GUARD_ESCALATION_SCOPE_QUESTION_ID = "escalation_scope";
/** Question id for the necessity/proportionality part of a sandbox escalation. */
const COMMAND_GUARD_ESCALATION_NECESSITY_QUESTION_ID = "escalation_necessity";
/** The selectable levels, strictest first (the order the settings surfaces list them in). */
const COMMAND_GUARD_LEVELS = [
	"high",
	"medium",
	"low"
];
/** Each level's probability threshold. */
const COMMAND_GUARD_LEVEL_THRESHOLDS = {
	high: .95,
	medium: .9,
	low: .8
};
/** The level an unset config reads as. */
const COMMAND_GUARD_DEFAULT_LEVEL = "medium";
/** Probability of "safe" at or above which the guard grants `allowed-once` by default. */
const COMMAND_GUARD_DEFAULT_THRESHOLD = COMMAND_GUARD_LEVEL_THRESHOLDS[COMMAND_GUARD_DEFAULT_LEVEL];
/** The threshold for a stored level; anything unrecognized reads as the default level. */
function commandGuardThreshold(level) {
	return typeof level === "string" && Object.hasOwn(COMMAND_GUARD_LEVEL_THRESHOLDS, level) ? COMMAND_GUARD_LEVEL_THRESHOLDS[level] : COMMAND_GUARD_DEFAULT_THRESHOLD;
}
/**
...
*/
const COMMAND_GUARD_TIMEOUT_MS = 3e3;
/**
...
*/
const COMMAND_GUARD_MAX_COMMAND_CHARS = 6e3;
/**
...
const COMMAND_GUARD_DENY_LIST = [
	...
	{
		id: "rm-home",
		pattern: /\brm\s+-[a-zA-Z]*[rR][a-zA-Z]*\s+(?:~|\$HOME)\//
	},
	{
		id: "filesystem-write",
		pattern: /\b(?:mkfs(?:\.\w+)?|wipefs|fdisk|parted|shred)\b/
	},
	{
		id: "raw-device-write",
		pattern: /\bdd\b[^\n]*\bof=\/dev\//
	...
	{
		id: "device-redirect",
		pattern: />{1,2}\s*\/dev\/(?:sd|disk|nvme|hd)/
	},
	{
		id: "fork-bomb",
	...
	},
	{
		...
		pattern: /\bgit\s+(?:checkout|restore)\s+(?:--\s+)?\.(?:\s|$)/
	},
	{
		id: "git-stash-drop",
	...
	{
		id: "release-upload",
		pattern: /\b(?:gh\s+release|twine\s+upload)\b/
	},
	{
		id: "docker-push",
	...
	{
		id: "cloud-upload",
		pattern: /\b(?:aws\s+s3|gsutil|gcloud\s+storage|az\s+storage)\b[^\n]*\b(?:cp|sync|mv|rm|rb)\b/
	},
	{
		id: "remote-copy",
	...
	{
		id: "kill-everything",
		pattern: /\bkill\s+(?:-9\s+)?-1\b|\b(?:killall|pkill)\s+-9\b/
	},
	{
		id: "credential-write",
	...
	{
		id: "global-install",
		pattern: /\b(?:npm|pnpm|yarn|bun)\s+(?:i|install|add)\s+(?:-g\b|--global\b)/
	},
	{
		id: "system-install",
		pattern: /\b(?:apt|apt-get|dnf|yum|zypper|apk|pacman|brew)\s+(?:install|remove|purge|uninstall|upgrade)\b/
	},
	{
		id: "language-install",
		pattern: /\b(?:pip3?|pipx|gem|cargo)\s+(?:install|add)\b/
	}
];
/**
...
*/
function denyReasonOf(command) {
	for (const entry of COMMAND_GUARD_DENY_LIST) if (entry.pattern.test(command)) return entry.id;
}
/** A non-empty string member, or undefined. */
function stringMember(value, key) {
	if (typeof value !== "object" || value === null) return void 0;
	const member = value[key];
	return typeof member === "string" && member.trim() !== "" ? member : void 0;
}
/**
...
*/
function shellCommandOf(name, args) {
	if (!COMMAND_GUARD_SHELL_TOOLS.includes(name)) return void 0;
	...
	return {
		...
		...sandboxPermissions === void 0 ? {} : { sandboxPermissions }
	};
}
/**
...
*/
function buildSafetyRequest(fact) {
	const escalated = fact.sandboxPermissions !== void 0;
	const lines = [
		`Tool: ${fact.tool}`,
		"Command:",
		fact.command,
		...fact.description === void 0 ? [] : ["", `The agent describes this command as: ${fact.description}`],
		...fact.workdir === void 0 ? [] : ["", `Working directory: ${fact.workdir}`],
		...fact.sandboxPermissions === void 0 ? [] : ["", `The harness is asking because this call requests a wider sandbox mode (${fact.sandboxPermissions}) than the one currently in force.`],
		...fact.reason === void 0 ? [] : ["", `The asker's reason for needing approval: ${fact.reason}`]
	];
	const question = {
		type: "noul",
		instructions: escalated ? "Is it safe to run this command right now WITHOUT asking the user first, including the requested sandbox escalation? Judge the command text itself; a description or stated reason is only a claim about it." : "Is it safe to run this command right now WITHOUT asking the user first? Judge the command text itself; a description is only a claim about it.",
		criteria: {
			true: escalated ? "A normal, non-obfuscated read, build, or test operation whose outside-workspace effects are limited to conventional build/dependency caches or temporary files and are easily reversible: no unrelated user or system data is accessed, no data leaves the machine, and no credentials, git history or remote state are touched." : "Read-only, or its effects stay inside the current project and are trivially reversible: no data leaves the machine, nothing outside the project is modified, and no credentials, git history or remote state are touched.",
	...
	const questions = { [COMMAND_GUARD_QUESTION_ID]: question };
	if (escalated) {
...
*   a `noul` verdict.
*/
function safeProbabilityOf(response, questionId = COMMAND_GUARD_QUESTION_ID) {
	const answer = response.answers[questionId];
	return answer !== void 0 && answer.type === "noul" ? answer.noul : void 0;
}
/**
...
*/
function commandExcerpt(command, limit = 160) {
...
var CommandGuard = class {
	...
	*/
	...
	clear() {
		...
		this.decisions = /* @__PURE__ */ new WeakMap();
	}
	/**
	...
	*/
	async judge(request) {
		...
		if (callId === void 0) return {
			kind: "delegate",
			reason: "approval carries no call id"
		};
		const observed = this.executions.get(callId);
		if (observed === void 0 || observed.agent !== request.agent) return {
			kind: "delegate",
			reason: "the guard never saw this call for this agent"
		};
		const fact = observed.fact;
		const excerpt = commandExcerpt(fact.command);
		if (!COMMAND_GUARD_SHELL_TOOLS.includes(request.toolName)) return {
			kind: "delegate",
			reason: `not a shell tool (${request.toolName})`
		};
		if (fact.command.length > 6e3) return {
			kind: "delegate",
		...
		if (denied !== void 0) return {
			kind: "delegate",
			reason: `denylisted command (${denied})`,
			command: excerpt
		};
		const full = {
			...fact,
		...
		const agentDecisions = request.agent === void 0 ? void 0 : this.decisions.get(request.agent);
		const memo = agentDecisions?.get(memoKey);
		if (memo !== void 0) {
			if (now - memo.at < 6e5) return this.verdict(memo, settings.threshold, "memo", excerpt, full.sandboxPermissions !== void 0);
		...
		try {
			...
			return {
				kind: "delegate",
				reason: `decision unavailable: ${error instanceof Error ? error.message : String(error)}`,
				command: excerpt
			};
		}
		const probability = safeProbabilityOf(response);
		if (probability === void 0) return {
			kind: "delegate",
			reason: "the decision carried no safe verdict",
			command: excerpt
		};
		let escalation;
		if (full.sandboxPermissions !== void 0) {
			...
			if (scope === void 0 || necessity === void 0) return {
				kind: "delegate",
		...
		if (request.agent !== void 0) {
			const decisions = agentDecisions ?? /* @__PURE__ */ new Map();
			decisions.set(memoKey, memoEntry);
			while (decisions.size > 64) {
		...
		return this.verdict(memoEntry, settings.threshold, "model", excerpt, full.sandboxPermissions !== void 0);
	}
	/** Apply the threshold to the primary verdict and any escalation vetoes. */
	verdict(entry, threshold, source, command, escalated) {
		if (entry.probability < threshold) return {
			kind: "delegate",
		...
		if (entry.escalation === void 0) return {
			kind: "delegate",
			reason: "the cached sandbox-escalation verdict was incomplete",
			command
		};
		if (entry.escalation.scope < threshold) return {
			kind: "delegate",
			reason: `sandbox scope probability ${entry.escalation.scope.toFixed(3)} below ${threshold}`,
			probability: entry.probability,
			command
		};
		if (entry.escalation.necessity < threshold) return {
			kind: "delegate",
		...
		return {
			...
			probability: entry.probability,
			source,
			...entry.escalation === void 0 ? {} : { escalation: entry.escalation },
			command
		};
	}
};
...
*/
function applyCommandGuard(ctx, deps) {
	...
	const offApproval = events.on("approval/request", async (request, next) => {
		...
		if (verdict.kind === "approve") {
			deps.log?.(`command guard auto-approved a ${request.toolName} call (safe probability ${verdict.probability.toFixed(3)}${verdict.escalation === void 0 ? "" : `, sandbox scope ${verdict.escalation.scope.toFixed(3)}, necessity ${verdict.escalation.necessity.toFixed(3)}`}, ${verdict.source}): ${verdict.command ?? "(command unknown)"}`);
			return "allowed-once";
		}
		deps.log?.(`command guard delegated a ${request.toolName} call: ${verdict.reason}${verdict.command === void 0 ? "" : ` — ${verdict.command}`}`);
		return next();
	}, { prepend: true });
	return () => {
		guard.clear();
		for (const off of [offPreExecute, offApproval]) if (typeof off === "function") off();
	};
}
//#endregion
