...
tests/client-boot.test.ts client-boot integration tests (real apply() against a
                      dsh 0.1.7-rc.1-shaped client assembly; settings page +
...
- **Isolated package install**: pnpm 10 auto-installs the package's DSH peers
  ...
  `@deepseek-ai/dsh-client-ui-dockkit` (read out of the 0.1.7-rc.1 engine) — so
  ...
  `dsh-client-ui-primitives@0.1.7-rc.1` declares NO dependencies at all, so
...
- **The Harness peer range names exactly ONE release, and that is
  ...
  `major.minor.patch` tuple as the comparator, so `^0.1.7-rc.1` resolves to
  `0.1.7-rc.1` and NOTHING else. That exactness is the point: this bundle is
...
- **Wire protocol** (reverse-engineered, command-code@1.28.4; re-verified unchanged against 1.65.0):
  ...
  - Catalog: `GET {apiBase}/provider/v1/models` → `{ object: 'list', data: [{ id, name, context_length, supported_endpoints }] }`. The `supported_endpoints` member is new as of command-code@1.55.0/1.56.0 and is the authoritative per-model route list (63 × `['/chat/completions','/responses']` as of command-code@1.65.0 — the 1.65.0 addition is chat-only and does not move this count — 9 Claude ids × `['/messages']` (`claude-opus-5-5` is the ninth), 9 × `['/chat/completions']` (8 plus `stealth/space-bunny-alpha`)); this adapter ignores it and keeps the `claude-*` prefix rule in `requiresMessagesEndpoint()`, which agrees with it today.
  - Provider API endpoints (docs, 2026-09-23): `/provider/v1/chat/completions`, `/provider/v1/responses` (OpenAI + open models; added in command-code@1.55.0), `/provider/v1/messages` (Claude family only), `/provider/v1/models`, and `/provider/v1/systemone` (System One decisions, model `typesafe/jev` — new in the docs and in the CLI bundle at command-code@1.64.0 and still present, with the same call site, in the 1.65.0 bundle; it is NOT a chat route, the model is absent from `/provider/v1/models`, and only `src/systemone.ts`'s own request path reaches it). This adapter's two chat transports stay the first plus `/alpha/generate`; neither is deprecated.
  ...
  - Defaults: `apiBase = https://api.commandcode.ai`, `COMMAND_CODE_CLI_VERSION = '1.65.0'`.
...
- **Plans & quota panel + composer session cost (ported from PR #36)**: two
  ...
  **The dashboard must stay escapable** (issue #41): the `main` cell occupies
  ...
  0.1.7-rc.1, but it has already been deleted once upstream while the row's
