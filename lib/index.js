...
*/
const COMMAND_CODE_CLI_VERSION = "1.64.0";
...
function toolResultOf(message) {
	...
	if (current.role === "tool") {
		if (typeof current.toolCallId !== "string" || current.toolCallId === "") return void 0;
		return {
	...
	if (kind !== void 0 && kind !== "tool") return void 0;
	...
	return {
		...
		fields: [
			...
			},
			{
				...
				hint: "typesafe/jev judges whether a shell command is safe and auto-approves it; everything else asks as usual. Off by default.",
			...
			},
			{
				...
				parse: (text) => {
					...
					return Number.isFinite(parsed) && parsed >= .5 && parsed <= 1 ? {
						kind: "set",
						value: parsed
					} : void 0;
				}
			},
			{
				...
				parse: (text) => {
					...
					if (trimmed === "") return { kind: "clear" };
					const parsed = Number(trimmed);
					return Number.isFinite(parsed) && parsed >= 200 && parsed <= 1e4 ? {
						kind: "set",
						value: parsed
					} : void 0;
				}
...
*
* The three question types are the whole vocabulary (command-code@1.64.0
...
const SYSTEMONE_DEFAULT_TIMEOUT_MS = 1500;
...
const COMMAND_GUARD_DEFAULT_TIMEOUT_MS = SYSTEMONE_DEFAULT_TIMEOUT_MS;
/** Bounds of the configurable probability, mirrored by the settings schemas. */
const COMMAND_GUARD_MIN_THRESHOLD = .5;
const COMMAND_GUARD_MAX_TIMEOUT_MS = 1e4;
/**
...
const COMMAND_GUARD_DENY_LIST = [
	...
	{
		id: "device-redirect",
		pattern: /(?:^|[^0-9\w])>{1,2}\s*\/dev\/(?:sd|disk|nvme|hd)/
	},
	{
		id: "fork-bomb",
...
var CommandGuard = class {
	...
	*/
	async judge(request) {
		...
		if (memo !== void 0) {
			if (now - memo.at < 6e5) return this.verdict(memo.probability, settings.threshold, "memo", excerpt);
	...
	verdict(probability, threshold, source, command) {
...
function applyCommandGuard(ctx, deps) {
	...
	const offApproval = events.on("approval/request", async (request, next) => {
		...
		if (verdict.kind === "approve") {
			deps.log?.(`command guard auto-approved a ${request.toolName} call (safe probability ${verdict.probability.toFixed(3)}, ${verdict.source}): ${verdict.command ?? "(command unknown)"}`);
			return "allowed-once";
		}
...
function unwrapVolatileConfig(config) {
	...
	let found = false;
	const plain = {};
	for (const [field, value] of Object.entries(source)) if (isVolatileRef(value)) {
		plain[field] = value.get();
		found = true;
	} else plain[field] = value;
	return found ? plain : config;
}
//#endregion
//#region src/index.ts
/**
...
function configFields() {
	return {
		...
		*/
		...
		commandGuardThreshold: z.number().min(COMMAND_GUARD_MIN_THRESHOLD).max(1),
		commandGuardTimeoutMs: z.number().min(200).max(COMMAND_GUARD_MAX_TIMEOUT_MS),
...
*/
const Config = z.object(markVolatileFields(configFields(), ["apiKey"]));
/**
...
*/
const LegacySettingsSchema = z.object(configFields());
/**
...
function apply(ctx, config) {
	...
	let source = () => config;
	const current = () => unwrapVolatileConfig(source());
	let lastRaw;
	let lastGood;
	const options = () => {
		const raw = current();
	...
	const commandGuardSettings = () => {
		...
		return {
			...
			threshold: raw.commandGuardThreshold ?? .9,
	...
	ctx.inject(["settings"], (settingsCtx) => {
		...
		if (typeof service.configure === "function") settingsCtx.effect(() => {
