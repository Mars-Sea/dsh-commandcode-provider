...
- **会话费用显示**：由 Command Code 服务的会话会在输入框下方的 token 计数旁显示本次会话已花费的美元金额，token 用量对话框的每一行也会显示对应价格。费率取自 Command Code 官方价格表，包含按小时计费的 DeepSeek 模型的峰时 / 非峰时价格。Harness 自身的 token / 缓存命中 / 吞吐量读数保持不变；没有可诚实定价的内容时（尚无用量、未知模型、或价格表未公布该费率）不会显示任何数字。同样固定为英文。
